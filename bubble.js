...
var Grader=function(inputFunction,submitURL,psetid,problemid,psetproblemid){
    ...
    //this.problemID=submitURL.split("/").slice(-2,-1)[0];
    //this.psetID=submitURL.split("/").slice(-3,-2)[0];
    this.problemID=problemid;
    this.psetID=psetid;
    this.psetProblemID=psetproblemid;
    //Added variables
...
Grader.prototype={
    ...
    updateCaseDetails:function(){
        ...
        if("input" in caseDetails){
            ...
            $details.find(".caseInput>pre").text(JSON.parse(caseDetails.input));
        }
        else{
        ...
        if("output" in caseDetails){
            ...
            $details.find(".caseOutput>pre").text(JSON.parse(caseDetails.output));
        }
        else{
        ...
        if("expected" in caseDetails){
            ...
            $details.find(".caseExpected>pre").text(JSON.parse(caseDetails.expected));
        }
        else{
...
var savePset=function(graders) {
    ...
    for(var i = 0 ; i < graders.length; i++) {
        if(currentProblem == i && currentProblem != null){
            graders[i].savedCode = valueFromSubmissionArea();
        }
        data["problem_"+graders[i].psetProblemID] = graders[i].savedCode;
    }
    $.ajax({
          url: saveURL,
          data: data,
...
var checkProblem=function(grader){
    ...
    if(currentProblem>0){
        $.extend(code,{
        "event":"endproblem",
        "problemID":grader.problemID,
        "userID":userID,
        "eventHash":grader.eventHash
    })
    }
    grader.timerDelay = (new Date()).valueOf();
    $.ajax({
        ...
        "success":function(response){
            grader.timerOffset = (new Date()).valueOf() - grader.timerDelay;
            grader.timerDelay = null;
            if(response){
...
var serverData=function(grader,serverData){
    if(grader.timerOffset !== undefined && grader.timerOffset>0) {
        grader.startTime = grader.startTime + grader.timerOffset;
        grader.timerOffset = 0;
    }
    grader.loading=0;
    grader.cases=[];
...
var switchToProblem=function(problemNumber){
    if(currentProblem==null||problemNumber!=currentProblem){
        var textrecording = problems[problemNumber].textrecording;
        var cleartext = false;
        if(textrecording !== undefined) {
            if(typeof switchTextRecording !== "undefined") {
                cleartext = switchTextRecording(textrecording, graders[problemNumber].problemID);
            }
        }
        ...
        if(graders[currentProblem].startTime==null){
            ...
            if(currentProblem>0){
                $.ajax({
                    ...
                    "data":{
                        "event":"startproblem",
                        "problemID":graders[currentProblem].problemID,
                        "userID":userID,
...
var updateTimer = function(){
    if(currentProblem!=null&&graders[currentProblem].startTime!=null && graders[currentProblem].timerDelay==null){
...
var init=function(data){
    ...
    for(var p=0;p<problems.length;p++){
        if(typeof problems[p] != 'object') continue;
        var newGrader=new Grader(valueFromSubmissionArea,problems[p].submitURL,problems[p].psetid,problems[p].problemid,problems[p].psetproblemid);
